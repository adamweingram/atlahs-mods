...
ASTRA_SIM_EXEC_SCRIPT = (
    "/workspace/apps/ai/astra-sim/examples/network_analytical/run_network_analytical.sh"
)
...
def write_real_runtime_to_file(
    real_runtime: float, data_dir: str, app_type: str, workload_name: str
) -> None:
    """
    ...
    with open(output_file, "a") as f:
        _ = f.write(f"{workload_name},{real_runtime}\n")
...
def get_real_runtime_for_ai_traces(trace_dir: str) -> float:
    ...
    if len(sqlite_files) != len(nsys_reports):
        ...
        assert os.system(cmd) == 0, (
            "Error converting the nsys reports to sqlite database files."
        )
    else:
        print_info(
            f"Found {len(sqlite_files)} sqlite database files. Skipping conversion..."
        )
...
def write_ai_workload_trace_sizes(
    goal_sizes: Dict[str, int], chakra_sizes: Dict[str, int]
) -> None:
    """
    ...
    """
    assert len(goal_sizes) == len(chakra_sizes), (
        "The number of goal and chakra sizes must be the same."
    )

    trace_size_file = os.path.join(OUTPUT_DIR, "ai", "trace_sizes.csv")
    with open(trace_size_file, "w") as f:
        _ = f.write("workload_name,goal_size,chakra_size\n")
        for workload_name in goal_sizes.keys():
            _ = f.write(
                f"{workload_name},{goal_sizes[workload_name]},{chakra_sizes[workload_name]}\n"
            )
...
def run_custom_exp(
    data_dir: str, app_type: str, overwrite: bool, verbose: bool
) -> None:
    """
    Runs the custom experiments.
    """
    print_info(f"Running the custom experiments for {app_type.upper()} workloads...")

    workload_dirs = get_workload_dirs(data_dir, app_type)
    astrasim_workload_dirs = (
        get_astrasim_workload_dirs(data_dir) if app_type == "ai" else []
    )
    if app_type == "ai":
    ...
    for workload_dir in workload_dirs:
        # Get the REAL (non-simulated) runtime for the workload
        real_runtime = get_real_runtime_for_workload(workload_dir, app_type)
        print_result(f"REAL runtime for {workload_dir}: {real_runtime} ns")
    ...
    if app_type == "ai":
        for workload_dir in astrasim_workload_dirs:
            ...
            assert os.system(astrasim_cmd) == 0, (
                f"Error running the AstraSim simulator for {workload_dir}."
            )
            ...
            print_info(
                f"Chakra size for {workload_name}: {chakra_sizes[workload_name]} MiB"
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run the validation experiment for the paper."
    )
    parser.add_argument(
        "-d",
        "--data-dir",
    ...
    run_custom_exp(args.data_dir, args.app_type, args.overwrite, verbose=True)
